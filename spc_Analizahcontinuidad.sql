...
Select * into #TempHcontinuidad from Hcontinuidad  with(nolock) where añoqna = @Qnaanterior
...
from Hcontinuidad with(nolock) inner join #TempHcontinuidad B                  
...
inner join #TempHcontinuidad b on b.id_emp = a.id_emp and charindex(','+ ltrim(rtrim(cast(b.id_plaza  as varchar))) + ',',',' + a.id_plazaanterior + ',') <> 0 
...
update a set a.añoQnaContinuidad = b.añoQnaContinuidad
from Hcontinuidad a with(nolock) 
inner join #TempHcontinuidad b on b.id_emp = a.id_emp and a.id_plaza = b.id_plaza
inner join empleados e with(nolock) on e.id_emp = a.id_emp
where a.[añoqna]= @Qna
...
from Hcontinuidad a with(nolock) inner join #TempHcontinuidad B                  
...
from Hcontinuidad a with(nolock) inner join (
  ...
  from Hcontinuidad a with(nolock)
  inner join (
  Select id_emp, añoqna, min(añoqnacontinuidad) añoqnacontinuidad  from #TempHcontinuidad group by id_emp,añoqna
...
-- en tipo, status y categoria          
update Hcontinuidad set Hcontinuidad.añoQnaContinuidad = b.añoQnaContinuidad                   
from Hcontinuidad with(nolock) inner join #TempHcontinuidad B on Hcontinuidad.id_emp = b.id_emp and Hcontinuidad.id_plaza = b.id_plaza 
inner join empleados e with(nolock) on e.id_emp = Hcontinuidad.id_emp
where Hcontinuidad.añoqna = @Qna and Hcontinuidad.id_plazaanterior = '' and          
Hcontinuidad.statusplaza = b.statusplaza and Hcontinuidad.tipoplaza = b.tipoplaza and           
Hcontinuidad.categoria = b.categoria 
